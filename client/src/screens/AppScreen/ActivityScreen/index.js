            ...
            </Text>
            {
                upcomingRides.length === 0 ?
                    <View style={styles.noRidesContainer}>
                        ...
                        <TouchableOpacity style={styles.reserveRideContainer} activeOpacity={0.8} onPress={handleBookRide}>
                    ...
                    <View style={styles.ridesContainer}>
                        {
                            upcomingRides.map((ride) => {
                                return (
                                    <UpcomingRideCard key={ride.id} date={ride.date} destination={ride.destination} type={ride.type} time={ride.time} />
                                )
                            })
                        }
                    </View>
                    ...
                    <View style={styles.ridesContainer}>
                        {
                            pastRides.map((ride) => {
                                return (
                                    <PastRideCard key={ride.id} date={ride.date} destination={ride.destination} type={ride.type} amount={ride.amount} time={ride.time} />
                                )
                            })
                        }
                        {/* <FlatList
