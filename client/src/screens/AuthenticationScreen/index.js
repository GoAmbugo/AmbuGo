...
function AuthenticationScreen({ navigation, route }) {
    ...
    const handleVerifyOTP = () => {
        if (!phone || phone.toString().length <= 9 || phone.toString().length >= 11) return setError('Please enter a valid phone number!')
        setError('')
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.phoneNumberContainer}>
                ...
                <AppNumberInput country={country} setCountry={setCountry} phone={phone} setPhone={setPhone} />
            </View>
            <ErrorText text={error} color='tomato' />
            <TouchableOpacity activeOpacity={0.9} style={styles.buttonContinue} onPress={handleVerifyOTP} >
